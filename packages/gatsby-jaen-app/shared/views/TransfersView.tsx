...
/**
 * "Preis festlegen und Angebot senden", the one press that prices a ride
 * and offers it (okf/architecture/offers-and-documents.md, "The price is
 * the offer"). The dialog names the two things the dispatcher is deciding
 * about, the amount the customer will read and the address it goes to, and
 * the pylon does the rest: it draws the number, compiles the offer with the
 * Typst engine in the Worker, uploads it to the storage gateway, stores the
 * OFFER document, mails it with the office copy and moves the ride to
 * OFFERED. No compiler is loaded in this browser for it; the browser's own
 * stays for the preview alone.
 *
 * A ride that is confirmed already is not offered again. There the dialog
 * is the plain "Preis festlegen" it always was, and the pylon makes the
 * confirmation's paper again with the new price behind it.
 */
export function PriceDialog({
  ...
  // An offer goes out on a NEW or an OFFERED ride and on no other.
  const offers = canSendOffer(transfer?.customerStatus)

  // The address the pylon will really use, passenger or account, the same
  // query the confirm dialog reads. Only asked while an offer would go out.
  const recipientQuery = useConfirmationRecipient(transfer?.id, open && offers)
  const answered = recipientQuery.query.data
  const recipient = answered
    ? answered.email.trim()
    : confirmationRecipient(transfer)
  const recipientPending = offers && recipientQuery.isLoading
  ...
  // What the offer will say, as it is typed, so the dispatcher reads the
  // number the customer will read and not the one that was stored.
  const typed = parseAmount(value)
  const amount = typed !== null && typed >= 0 ? formatAmount(code, typed) : null
  ...
  const submit = async () => {
    ...
    try {
      ...
      toaster.success({
        title: offers ? t.ToastOfferSent : t.ToastPriceSet
      })
      onSaved(row)
      onClose()
    } catch (err) {
  ...
  return (
    <Sheet
      open={open}
      onClose={onClose}
      title={offers ? t.PriceOfferTitle : t.PriceTitle}
      size="sm"
      busy={busy}
      footer={
        <DialogActions
          onCancel={onClose}
          confirmLabel={offers ? t.PriceOfferSubmit : t.PriceSubmit}
          onConfirm={submit}
          loading={busy}
        />
      }>
      <Stack gap="3">
        ...
        {offers && (
          <Stack gap="1" data-testid="price-offer-hint">
            <Text textStyle="sm" color="fg.muted">
              {t.PriceOfferBody}
            </Text>
            <Text textStyle="sm" fontWeight="medium">
              {amount
                ? fill(t.PriceOfferAmount, {price: amount})
                : t.PriceOfferNoAmount}
            </Text>
            <Text
              textStyle="sm"
              color={recipient || recipientPending ? 'fg.muted' : 'fg.error'}
              data-testid="price-offer-recipient">
              {recipient
                ? fill(t.PriceOfferRecipient, {recipient})
                : recipientPending
                  ? t.ConfirmBookingRecipientPending
                  : t.PriceOfferNoRecipient}
            </Text>
          </Stack>
        )}
      </Stack>
...
export interface CreateForm {
  ...
  pickupTime: string
  /**
   * "Vergangene Fahrt nachtragen", dispatch.md section 12. Off by default:
   * the office enters rides that are still to come, and a backdated one is a
   * deliberate act that the pylon otherwise refuses with PICKUP_IN_PAST.
   */
  allowPast: boolean
  subject: string
  firstName: string
...
export function CreateTransferDialog({
  ...
  // No ride in the past, dispatch.md section 12. The office may still write
  // one, but only with the tick: the same rule the pylon holds, asked here so
  ...
  const submit = async () => {
    ...
    const input: CreateTransferInput = {
      ...
      allowPast: form.allowPast || undefined,
      ...
      <Stack gap="6">
        ...
        <FormSection title={t.SecRoute}>
          ...
          <Stack direction={{base: 'column', md: 'row'}} gap="3">
            <Field.Root
              required
              invalid={touched && (missing.pickupDateTime || pickupInPast)}>
              <Field.Label>
              ...
              <Field.ErrorText>
                {missing.pickupDateTime ? t.DateInvalid : t.DatePast}
              </Field.ErrorText>
              <Switch.Root
                mt="2"
                size="sm"
                checked={form.allowPast}
                onCheckedChange={e => set('allowPast', e.checked)}>
                <Switch.HiddenInput />
                <Switch.Control />
                <Switch.Label textStyle="sm">{t.LabelBackdate}</Switch.Label>
              </Switch.Root>
              <Field.HelperText>{t.BackdateHint}</Field.HelperText>
            </Field.Root>
            <Field.Root>
...
export interface RowActions {
  ...
  /**
   * The row after the signed-in person answered a request from the card
   * (dispatch.md section 9a). Without it the card draws no answer buttons.
   */
...
/**
 * The answer, wherever the person who has to give it stands (dispatch.md
 * section 9a). Until now the yes and the no lived on the driver's own ride
 * screen alone, so a person whose first role is admin saw the dispatcher's
 * screens and could not answer a request meant for them.
 *
 * Two cases, and the component draws whichever applies:
 *
 *   the requested driver, whatever their other roles, gets "Anfrage annehmen"
 *   and "Ablehnen";
 *   an admin who is not the requested driver gets "Fuer den Fahrer annehmen"
 *   on any open request, the yes taken on the phone. The pylon records them
 *   as the attempt's answeredBy.
 *
 * A ride with no open request draws nothing. `reason` is offered where there
 * is room for a field (the detail page); the card declines without one, the
 * reason belongs to a conversation and the ride page is one tap away.
 */
export function DriverAnswerActions({
  transfer,
  onAnswered,
  withReason = false,
  ...rest
  ...
  onAnswered: (row: TransferRow) => void
  /** Draw the reason field above the buttons, for a screen with the room. */
  withReason?: boolean
} & StackProps) {
  const {t} = useTransferStrings()
  ...
  const open = transfer.driverStatus === 'REQUESTED'
  const mine = open && !!caller.userId && transfer.driverId === caller.userId
  const onBehalf = open && !mine && caller.isAdmin
  if (!mine && !onBehalf) return null
  ...
  const answer = async (kind: 'accept' | 'decline') => {
    ...
    try {
      const row =
        kind === 'accept'
          ? await acceptAssignment(transfer.id)
          : await declineAssignment(transfer.id, reason || undefined)
      onAnswered(row)
      toaster.success({
        title: kind === 'accept' ? t.ToastAccepted : t.ToastDeclined
      })
    } catch (err) {
      setError(errorMessage(err, t.AnswerFailed))
    } finally {
      setBusy(null)
    }
  }
  ...
  return (
    <Stack gap="3" data-testid="driver-answer" {...rest}>
      <Text textStyle="sm" fontWeight="medium">
        {mine ? t.AnswerYoursTitle : t.AnswerForDriverHint}
      </Text>
      {error && <ErrorBanner message={error} />}
      {mine && withReason && (
        <Stack gap="1">
      ...
      {/* Two controls in a row are never closer than a thumb, and never
          smaller than 44 px where a thumb is what taps them. */}
      <Stack direction={{base: 'column', sm: 'row'}} gap="3">
        <Button
          ...
          {mine ? t.AnswerAccept : t.AnswerForDriver}
        </Button>
        {mine && (
          <Button
            size="sm"
  ...
  return (
    <Box
      ...
      {/*
        ...
        says yes or no from the board without opening the ride, and an admin
        takes the yes on the driver's behalf. Drawn on the driver's own list
        as well, which is the same card with the dispatcher's cells hidden.
      */}
      {actions.onAnswered && (
        <Box mt="3" onClick={e => e.stopPropagation()}>
          <DriverAnswerActions transfer={row} onAnswered={actions.onAnswered} />
        </Box>
      )}
...
function MyRides({heading, subtitle}: {heading: string; subtitle?: string}) {
  ...
  const actions = useMemo<RowActions>(
    () => ({
      onOpen: row => navigate(transferPath(row)),
      // The driver answers from their own list too, and the list is read
      // again afterwards: it has no row replacement of its own.
